...
\begin{equation}\label{eq:coord-inverso-P}
...
\begin{comment}
Por ejemplo, si $E$ está dada por la ecuación $y^2=x^3+ax^2+bx$, y $C$ el subgrupo de orden 2 generado por $T=(0,0)$, entonces
\[
	X(P)=x(P)+x(P+T)-x(T)=x(P)+\paren{\frac{y(P)-y(T)}{x(P)-x(T)}}^2-a-x(P)-x(T)=\paren{\frac{y(P)}{x(P)}}^2-a,
\]
porque $T=(x(T),y(T))=(0,0)$. Si escribimos $P=(x,y)$, entonces tenemos:
\[
	X(x,y)=\frac{y^2}{x^2}-a=\frac{y^2-ax^2}{x^2}=\frac{x^3+bx}{x^2}=x+\frac{b}{x}.
\]
Para calcular $Y(x,y)$, solamente necesitamos calcular $y(P+T)$. Como $x(P+T)=b/x$ entonces:
\begin{gather*}
	y(P+T)^2=\frac{b^3}{x^3}+a\frac{b^2}{x^2}+b\frac{b}{x}\quad\then\quad y(P+T)^2= b^3x^{-3}\left(1+\frac{x}{b}a+\frac{x^2}{b^2}b\right)\\
	\therefore y(P+T)=\frac{1+ab^{-1}x+b^{-1}x^2}{x^3}.
\end{gather*}
Por lo tanto
\[
	Y(x,y)=y+\frac{1+ab^{-1}x+b^{-1}x^2}{x^3}.
\]
\end{comment}
\end{nota}
...
\begin{defin}\label{def:reduccion}
...
Por otro lado para cada $\p$ primo existe un cambio de variable que lleva la ecuación de $E$, que tiene coeficientes en $K$, a una ecuación de Weierstrass con coeficientes en $\Oo$:
...
\begin{defin}\label{def:conductor}
...
El tipo de reducción en el que vamos a estar interesados es en la reducción buena o mulitplicativa:
